...
def generate_reply(input, sc_num):
    ...
    for i in range(sc_num):
        ...
        try:
            ...
            raw_tab_col = '{' + raw_tab_col.split('{')[-1]
            raw_tab_col = raw_tab_col.rsplit('}', 1)[0] + '}'
            raw_tab_col = json.loads(raw_tab_col)
        except:
            print('list error')
            print(f'raw_tab_col: {raw_tab_col}')
            lines = raw_tab_col.splitlines();
            cleaned_lines = [line.split('//')[0] for line in lines if not line.strip().startswith('//')];
            raw_tab_col = '\n'.join(cleaned_lines);
            try:
                raw_tab_col = json.loads(raw_tab_col)
            except:
                try:
                    raw_tab_col = json.loads(raw_tab_col.replace(",\n  }", "\n  }"))  # dấu , cuối cùng ở json
                except:
                    raw_tab_col = {}
...
2 - Column names could be ambiguous, and synonyms. So you need to output all the columns that you think are relevant to the question, even if they are not explicitly mentioned in the question.
