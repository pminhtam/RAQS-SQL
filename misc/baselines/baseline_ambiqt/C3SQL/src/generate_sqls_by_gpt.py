...
Gold SQL: [select A from B group by A order by count ( * ) desc limit 1; select synonym_A from B group by synonym_A order by count ( * ) desc limit 1; ]
        ...
        for j in range(5):
            ...
            while reply is None:
                try:
                    ...
                    time.sleep(3)
                    pass
        item["C3SQL_queries_text"] = reply
        results.append(item)
        # time.sleep(1)
    # p_sql_final = get_sqls(results, opt.n, opt.db_dir)

    # try:
    #     json.dump(results, open(opt.output_dataset_path, "w"), indent=4)
    # except:
    #     import pdb; pdb.set_trace()
