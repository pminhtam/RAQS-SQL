...
def replace_db_schema_extra_map(preprocessed_data, extra_map):
    """
    Replace the db_schema with the extra_map.
    :param db_schema: The original db_schema.
    :param extra_map: The extra map to replace the db_schema.
    :return: The replaced db_schema.
    """
    db_schema = preprocessed_data["db_schema"].copy()
    for table_item in db_schema:
        table_name_original = table_item["table_name_original"]
        if table_name_original in extra_map:
            ori_col_in_extra_map = list(extra_map[table_name_original].keys())[0]

            idx_ori_col = table_item["column_names_original"].index(ori_col_in_extra_map)
            table_item["column_names_original"][idx_ori_col] = extra_map[table_name_original][ori_col_in_extra_map][0]
            table_item["column_names"][idx_ori_col] = extra_map[table_name_original][ori_col_in_extra_map][0]
            column_type_amb = table_item["column_types"][idx_ori_col]
            db_content_amb = table_item["db_contents"][idx_ori_col]
            table_item["column_names_original"].append(extra_map[table_name_original][ori_col_in_extra_map][1])
            table_item["column_names"].append(extra_map[table_name_original][ori_col_in_extra_map][1])
            table_item["column_types"].append(column_type_amb)
            table_item["db_contents"].append(db_content_amb)
            # import pdb;
            # pdb.set_trace()
    preprocessed_data["db_schema"] = db_schema
    return preprocessed_data
def replace_db_schema_extra_table_map(preprocessed_data, extra_table_map):
    """
    Replace the db_schema with the extra_map.
    :param db_schema: The original db_schema.
    :param extra_map: The extra map to replace the db_schema.
    ...
    for table_item in db_schema:
        table_name_original = table_item["table_name_original"]
        if table_name_original in extra_table_map:
            # ori_col_in_extra_map = extra_table_map[table_name_original][0]
            # import pdb; pdb.set_trace()
            db_schema.append({
                "table_name_original": extra_table_map[table_name_original][1],
                "table_name": extra_table_map[table_name_original][1],
                "column_names": table_item["column_names"].copy(),
                "column_names_original": table_item["column_names_original"].copy(),
                "column_types": table_item["column_types"].copy(),
                "db_contents": table_item["db_contents"]
            })
            table_item["table_name_original"] = extra_table_map[table_name_original][0]
            table_item["table_name"] = extra_table_map[table_name_original][0]
    ...
    for natsql_data, data in tqdm(zip(natsql_dataset, dataset)):
        ...
        for table in db_schemas[db_id]["schema_items"]:
            db_contents = get_db_contents(
                ...
                table["column_names_original"].copy(),
                db_id,
                opt.db_path
            )
            ...
            preprocessed_data["db_schema"].append({
                ...
                "column_names": table["column_names"].copy(),
                "column_names_original": table["column_names_original"].copy(),
                "column_types": table["column_types"].copy(),
        ...
        # import pdb;
        # pdb.set_trace()
        # preprocessed_data = replace_db_schema_extra_map(preprocessed_data, data.get("extra_map", {}))
        preprocessed_data = replace_db_schema_extra_table_map(preprocessed_data, data.get("extra_table_map", {}))
