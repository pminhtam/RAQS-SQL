...
if __name__ == "__main__":
    ...
    for id, data in enumerate(data_all):
        data['input_sequence'] = "### Complete all probable sqlite SQL queries only and with no explanation, and do not select extra columns that are not explicitly requested in the query. " \
                                 "\n Output is list of probable sqlite SQL queries " \
        ...
        for tab, cols in data['schema'].items():
            ...
            for i, col in enumerate(cols):
                schema += col
                if i > len(data['db_contents'][tab])-1:
                    continue
