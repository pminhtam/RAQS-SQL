...
2 - Table names could be ambiguous, and synonyms. So you need to output all the tables that you think are relevant to the question, even if they are not explicitly mentioned in the question.
3 - Check whether you consider all the tables.
4 - Output a list object in the order of step 3, Your output should contain all the tables. The format should be like: 
[
    "table_1", "table_2", ...
]
    ...
    for i, data in enumerate(tqdm(data_all)):
        ...
        while tables_all is None:
            try:
                ...
                # print(tables_all)
                # print(f'get {len(tables_all)} tables from api')
            except:
                print(f'api error, wait for 3 seconds and retry...')
                time.sleep(3)
                pass
                # tables_all = [[]*sc_num]  # if api error, return empty list for each sc_num
        tables_ori = []
        for table in data['db_schema']:
