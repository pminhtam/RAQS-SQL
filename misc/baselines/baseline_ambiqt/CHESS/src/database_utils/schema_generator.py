...
class DatabaseSchemaGenerator:
    ...
    def _load_schema_into_cache(cls, db_id: str, db_path: str) -> None:
        ...
        """
        """
        Thực hiện execute SQL để lấy value examples.
        CACHED_DB_SCHEMA : đến db_id nào thì lưu vào để sau dùng luôn, đỡ phải load lại schema.
        """
    ...
    def generate_schema_string(self, include_value_description: bool = True, shuffle_cols: bool = True, shuffle_tables: bool = True, extra_map: Dict[str, Any] = {},extra_table_map: Dict[str, Any] = {}) -> str:
        """
        ...
        """
        đây mới là ham chính để đưa vào generate SQL nè
        """
        ...
        for table_name, ddl_command in ddl_commands.items():
            ...
            if table_name.lower() in extra_map:       # Có thông tin về synonym column
                column_synonym_map = extra_map[table_name.lower()]  # map {ori_column_name: [synonym_column_name1, synonym_column_name2, ...]}
            else:
                column_synonym_map = {}
            for column_def in definitions:
                ...
                if any(keyword in column_def.lower() for keyword in ["foreign key", "primary key"]):
                    ...
                    schema_lines_col = []
                    if column_name.lower() in column_synonym_map:
                        for synonym_col in column_synonym_map[column_name.lower()]:
                            new_column_def = column_def.replace(column_name, synonym_col, 1)
                            if (column_name in targeted_columns) or self._is_connection(table_name, column_name):
                                new_column_def += self._get_example_column_name_description(table_name, column_name,
                                                                                            include_value_description)
                                schema_lines_col.append(new_column_def)
                            elif column_def.lower().startswith("unique"):
                                schema_lines_col.append(new_column_def)
                    elif (column_name in targeted_columns) or self._is_connection(table_name, column_name):
                        new_column_def = f"\t{column_def},"
                        ...
                        new_column_def = f"\t{column_def},"
                        schema_lines_col.append(new_column_def)
                    schema_lines.extend(schema_lines_col)
            schema_lines.append(");")
            ddl_commands[table_name] = '\n'.join(schema_lines)
        ddl_commands_new = {}
        for table_name in ddl_commands:
            if table_name.lower() in extra_table_map:
                for table_name_text_syn in extra_table_map[table_name.lower()]:
                    if table_name_text_syn.lower() != table_name.lower():
                        ddl_commands_new[table_name_text_syn] = ddl_commands[table_name].replace(f"CREATE TABLE {table_name}", f"CREATE TABLE {table_name_text_syn}")
            else:
                ddl_commands_new[table_name] = ddl_commands[table_name]
        # import pdb; pdb.set_trace()
        return "\n\n".join(ddl_commands_new.values())

    def get_column_profiles(self, with_keys: bool = False, with_references: bool = False, extra_map: Dict[str, Any] = {},extra_table_map: Dict[str, Any] = {}) -> Dict[str, Dict[str, str]]:
        """
        ...
        """
        """
        Thay cái này bằng synonym_column nè 
        """
        column_profiles = {}
        # import pdb; pdb.set_trace()

        for table_name, table_schema in self.schema_structure.tables.items():
            if table_name.lower() in extra_table_map:       # Có thông tin về synonym table
                table_name_text = extra_table_map[table_name.lower()][0]
            else:
                table_name_text = table_name
            column_profiles[table_name_text] = {}
            for column_name, column_info in table_schema.columns.items():
                # if table_name_text.lower() in extra_map and column_name.lower() in extra_map[table_name_text.lower()]:        # Column synonym
                if False:        # Column synonym
                    # import pdb; pdb.set_trace()
                    column_name_synonym = extra_map[table_name_text.lower()][column_name.lower()]
                    for column_name_text in column_name_synonym:
                        if with_keys or not (column_info.primary_key or column_info.foreign_keys or column_info.referenced_by):
                            column_profile = f"Table name: `{table_name_text}`\nOriginal column name: `{column_name_text}`\n"
                            if column_info.type:
                            ...
                            column_profiles[table_name_text][column_name_text] = column_profile
                else:       # Không có thông tin về synonym column thì chạy như cũ
                    if with_keys or not (column_info.primary_key or column_info.foreign_keys or column_info.referenced_by):
                        ...
                        if with_references:
                            ...
                            if column_info.referenced_by:
                                ...
                                for source_table, source_column in column_info.referenced_by:
                                    column_profile += f"    Table: `{source_table}`, Column: `{source_column}`\n"
                        column_profiles[table_name_text][column_name] = column_profile
            # if table_name.lower() in extra_table_map:       # Copy cho synonym table
            #     for table_name_text_syn in extra_table_map[table_name.lower()]:
            #         if table_name_text_syn.lower() != table_name_text.lower():
            #             column_profiles[table_name_text_syn] = column_profiles[table_name_text].copy()
            #             for column_name, column_profile in column_profiles[table_name_text_syn].items():
            #                 column_profiles[table_name_text_syn][column_name] = column_profile.replace(f"Table name: `{table_name_text}`", f"Table name: `{table_name_text_syn}`")
