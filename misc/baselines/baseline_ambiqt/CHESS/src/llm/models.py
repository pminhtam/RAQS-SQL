...
def call_api_llm_chain(prompt: Any, engine: Any, request_kwargs: Dict[str, Any]):
    chain = prompt | engine
    prompt_text = prompt.invoke(request_kwargs).messages[0].content
    output_llm_api = chain.invoke(request_kwargs)
    return output_llm_api, prompt_text
...
def call_llm_chain(prompt: Any, engine: Any, parser: Any, request_kwargs: Dict[str, Any], step: int, max_attempts: int = 12, backoff_base: int = 2, jitter_max: int = 60) -> Any:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            output, prompt_text = call_api_llm_chain(prompt, engine, request_kwargs)
            ...
            logger.log(f"Function timed out: {e}", "error")
