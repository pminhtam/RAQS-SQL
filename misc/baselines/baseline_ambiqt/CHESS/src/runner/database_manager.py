...
class DatabaseManager:
    ...
    def __new__(cls, db_mode=None, db_id=None, extra_map: Dict[str, Any] = None,extra_table_map: Dict[str, Any] = None):
        if (db_mode is not None) and (db_id is not None):
            with cls._lock:
                if cls._instance is None:
                    cls._instance = super(DatabaseManager, cls).__new__(cls)
                    cls._instance._init(db_mode, db_id, extra_map,extra_table_map)
    ...
    def _init(self, db_mode: str, db_id: str, extra_map: Dict[str, Any] = None,extra_table_map: Dict[str, Any] = None):
        """
        ...
        self.extra_table_map = extra_table_map if extra_table_map is not None else {}
        self._set_paths()
        self.lsh = None
        self.minhashes = None
        self.vector_db = None
    ...
    def get_column_profiles(self, schema_with_examples: Dict[str, Dict[str, List[str]]],
        ...
        # import pdb; pdb.set_trace()
        try:
            column_profiles = database_schema_generator.get_column_profiles(with_keys, with_references, extra_map=self.extra_map,extra_table_map=self.extra_table_map)
            # import pdb; pdb.set_trace()
        except Exception as e:
            print(f"Error generating column profiles for {self.db_id}: {e}")
            column_profiles = {}
            import pdb; pdb.set_trace()
    ...
    def get_database_schema_string(self, tentative_schema: Dict[str, List[str]], 
                                   ...
                                   include_value_description: bool, use_synonym: bool = False) -> str:
        """
        ...
        if use_synonym:
            schema_string = schema_generator.generate_schema_string(include_value_description=include_value_description, extra_map=self.extra_map,extra_table_map=self.extra_table_map)
        else:
