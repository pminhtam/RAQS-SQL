...
class RunManager:
    ...
    def get_result_directory(self) -> str:
        ...
        dataset_name = Path(self.args.data_path).parent.stem
        # run_folder_name = str(self.args.run_start_time)       # bỏ time ra khỏi folder name
        # run_folder_name = "test_run"
        # run_folder_name = "ambiqt_col"
        run_folder_path = Path(self.RESULT_ROOT_PATH) / data_mode / setting_name / dataset_name
    ...
    def initialize_tasks(self, dataset: List[Dict[str, Any]]):
        ...
        for i, data in enumerate(dataset):
            ...
            data['evidence'] = ""
            if "orig_query" in data:
                data['SQL'] = data["orig_query"]
            task = Task(**data)
    ...
    def run_tasks(self):
        ...
        if self.args.num_workers > 1:
            ...
            f_out_jsonl = open(os.path.join(self.result_directory, "ambiqt.jsonl"), "a+")
            idx = 0
            for task in self.tasks:
                # import pdb; pdb.set_trace()
                idx += 1
                ...
                for step in state.execution_history:
                    if step["tool_name"] == "generate_candidate":
                        if "candidates" in step and len(step["candidates"]) > 0:
                            for i in range(len(step["candidates"])):
                                sql = step["candidates"][i]["SQL"]
                                queries.append(sql)
                # state = system_state[0]
                output_item = {
                    "db_id": task.db_id,
                    "orig_example_idx": task.orig_example_idx,
                    "orig_query" : task.orig_query,
                    "extra_map": task.extra_map,
                    "extra_table_map": task.extra_table_map,
                    "queries": queries,
                }
                # import pdb; pdb.set_trace()
                f_out_jsonl.write(json.dumps(output_item) + "\n")
                print(f"Task {idx} done in {time.time() - start_time:.2f} seconds. at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
                print("Process done   : ",idx, " *************************", task.question)
            f_out_jsonl.close()
    ...
    def worker(self, task: Task) -> Tuple[Any, str, int]:
        ...
        DatabaseManager(db_mode=self.args.data_mode, db_id=task.db_id, extra_map=task.extra_map, extra_table_map= task.extra_table_map)
        ...
        # import pdb; pdb.set_trace()
        # for state_dict in team.stream(state_values, thread_config):
        # import pdb; pdb.set_trace()
        # for state_dict in team.stream(state_values, thread_config, stream_mode="values"):
        for state_dict in team.stream(state_values, stream_mode="values"):
    ...
    def pick_final_sql(self, state: SystemState):
        ...
        """
        print("Picking final SQL from execution history")
