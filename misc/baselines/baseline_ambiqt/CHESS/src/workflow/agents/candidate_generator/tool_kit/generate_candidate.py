...
from llm.models import async_llm_chain_call, get_llm_chain, call_llm_chain
...
class GenerateCandidate(Tool):
    ...
    def _run(self, state: SystemState):
        ...
        """
        print("Running GenerateCandidate tool...")
        ...
        for generator_config in self.generator_configs:
            ...
            for i in range(generator_config.sampling_count):
                try:
                    request_kwargs = {
                        "DATABASE_SCHEMA": state.get_schema_string(schema_type="tentative_with_synonyms"),
