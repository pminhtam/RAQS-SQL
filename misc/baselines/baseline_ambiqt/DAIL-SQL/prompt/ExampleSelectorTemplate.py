...
class EuclideanDistanceQuestionMaskSelector(BasicExampleSelector):
    def __init__(self, data, *args, **kwargs):
        ...
        self.SELECT_MODEL = "sentence-transformers/all-mpnet-base-v2"
        # self.SELECT_MODEL = "sentence-transformers/all-MiniLM-L6-v2"
        self.mask_token = "<mask>"  # the "<mask>" is the mask token of all-mpnet-base-v2
        self.value_token = "<unk>" # the "<unk>" is the unknown token of all-mpnet-base-v2
        print("Run selector type EuclideanDistanceQuestionMaskSelector")
        from sentence_transformers import SentenceTransformer
        train_mask_questions = mask_question_with_schema_linking(self.train_json, mask_tag=self.mask_token, value_tag=self.value_token)
        self.bert_model = SentenceTransformer(self.SELECT_MODEL, device="cpu")
        self.train_embeddings = self.bert_model.encode(train_mask_questions)
        self.load_ambiqt_train()
        # import pdb; pdb.set_trace()
    def load_ambiqt_train(self):
        self.ambiqt_train_dict = {}
        if self.data.ambiqt_train_json is not None:
            ambiqt_train_json = json.load(open(self.data.ambiqt_train_json, 'r'))
            for item in ambiqt_train_json:
                if item["question"].lower() not in self.ambiqt_train_dict:
                    self.ambiqt_train_dict[item["question"].lower()] = item
                    self.ambiqt_train_dict[item["question"].lower()]["query"] = f"[ {item['query1']} ; {item['query2']} ]"        # [item["query1"], item["query2"]]
                    # self.ambiqt_train_dict[item["question"].lower()]["query"] = f"\"{item['query1']}\" ; \"{item['query2']}\""        # [item["query1"], item["query2"]]
    def get_examples(self, target, num_example, cross_domain=False):
        target_mask_question = mask_question_with_schema_linking([target], mask_tag=self.mask_token, value_tag=self.value_token)
        target_embedding = self.bert_model.encode(target_mask_question)
        ...
        # import pdb; pdb.set_trace()
        output_example = []
        for index, d in top_pairs:
            question = train_json[index]["question"]
            if question.lower() in self.ambiqt_train_dict:
                # if the question is in ambiqt train, then use the ambiqt train item
                ambiqt_item = self.ambiqt_train_dict[question.lower()]
                ambiqt_item["query_skeleton"] = train_json[index]["query_skeleton"]
                ambiqt_item["question_pattern"] = train_json[index]["question_pattern"]
                output_example.append(ambiqt_item)
        # import pdb; pdb.set_trace()
        # return [train_json[index] for (index, d) in top_pairs]
