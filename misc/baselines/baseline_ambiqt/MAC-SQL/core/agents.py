...
class Selector(BaseAgent):
    ...
    def _load_single_db_info(self, db_id: str,
                             extra_map: dict, extra_table_map: dict) -> dict:
        ...
        # time.sleep(3)
        if len(extra_map) > 0:
            # replace original column with synonym column in extra_map
            table2coldescription_ori = deepcopy(table2coldescription)
            table_unique_column_values_ori = deepcopy(table_unique_column_values)
            try:
                table2coldescription, table_unique_column_values = self.replace_db_schema_extra_map(table2coldescription, table_unique_column_values, extra_map)
            except:
                import pdb; pdb.set_trace()
        if len(extra_table_map) > 0:
            # replace original table with synonym table in extra_table_map
            table2coldescription_ori = deepcopy(table2coldescription)
            table_unique_column_values_ori = deepcopy(table_unique_column_values)
            table2primary_keys_ori = deepcopy(table2primary_keys)
            table_foreign_keys_ori = deepcopy(table_foreign_keys)
            try:
                table2coldescription, table_unique_column_values, table2primary_keys, table_foreign_keys = self.replace_db_schema_extra_table_map(table2coldescription, table_unique_column_values, table2primary_keys, table_foreign_keys, extra_table_map)
            except:
                import pdb; pdb.set_trace()
        # import pdb; pdb.set_trace()
        ...
        return result
    def replace_db_schema_extra_table_map(self, table2coldescription, table_unique_column_values,table2primary_keys,table_foreign_keys, extra_table_map):
        for ori_table_name_in_map in extra_table_map:
            ori_table_name = ori_table_name_in_map
            for table_name in table2coldescription:  # Tránh trường hợp tên có chữ hoa chữ thường khác nhau
                if ori_table_name_in_map.lower() == table_name.lower():
            ...
            for synonym_table in list_synonym_tables:
                table2coldescription[synonym_table] = table2coldescription[ori_table_name]
                table_unique_column_values[synonym_table] = table_unique_column_values[ori_table_name]
                table2primary_keys[synonym_table] = table2primary_keys[ori_table_name]
                table_foreign_keys[synonym_table] = table_foreign_keys[ori_table_name]
            del table2coldescription[ori_table_name]
            del table_unique_column_values[ori_table_name]
            del table2primary_keys[ori_table_name]
            del table_foreign_keys[ori_table_name]
        return table2coldescription, table_unique_column_values, table2primary_keys, table_foreign_keys

    def replace_db_schema_extra_map(self, table2coldescription, table_unique_column_values, extra_map):
        """
        Replace the original column with synonym column in extra_map
        """
        for ori_table_name_in_map in extra_map:
            ...
            for ori_column_name in extra_map[ori_table_name_in_map]:
                list_synonym_columns = extra_map[ori_table_name_in_map][ori_column_name]
                table2coldescription_replaced_item = []
                table_unique_column_values_replaced_item = []
                for item in table2coldescription[ori_table_name]:
                    col_name, full_col_name, extra_desc = item
                    if col_name.lower() == ori_column_name.lower():
                        for synonym_col in list_synonym_columns:
                            table2coldescription_replaced_item.append([synonym_col, synonym_col, extra_desc])
                    else:
                        table2coldescription_replaced_item.append(item)
                table2coldescription[ori_table_name] = table2coldescription_replaced_item
                for item in table_unique_column_values[ori_table_name]:
                    col_name, value_examples_str = item
                    if col_name.lower() == ori_column_name.lower():
                        for synonym_col in list_synonym_columns:
                            table_unique_column_values_replaced_item.append([synonym_col, value_examples_str])
                    else:
                        table_unique_column_values_replaced_item.append(item)
                table_unique_column_values[ori_table_name] = table_unique_column_values_replaced_item
        return table2coldescription, table_unique_column_values
    ...
    def _get_db_desc_str(self,
                         ...
                         extra_map: dict, extra_table_map: dict,
        ...
        if self.db2infos.get(db_id, {}) == {}:  # lazy load
            self.db2infos[db_id] = self._load_single_db_info(db_id, extra_map=extra_map, extra_table_map=extra_table_map)
    ...
    def talk(self, message: dict):
        ...
        extra_map, extra_table_map = message.get('extra_map', {}), \
                                    message.get('extra_table_map', {})
        use_gold_schema = False
        if ext_sch:
            use_gold_schema = True
        db_schema, db_fk, chosen_db_schem_dict = self._get_db_desc_str(db_id=db_id, extracted_schema=ext_sch, use_gold_schema=use_gold_schema,extra_map=extra_map, extra_table_map=extra_table_map)
        ...
        if ext_sch == {} and need_prune:
            ...
            db_schema_str, db_fk, chosen_db_schem_dict = self._get_db_desc_str(db_id=db_id, extracted_schema=raw_extracted_schema_dict,extra_map=extra_map, extra_table_map=extra_table_map)
...
class Decomposer(BaseAgent):
    ...
    def talk(self, message: dict):
        ...
        message['ori_decomposer_reply'] = reply
...
class Refiner(BaseAgent):
    ...
    def talk(self, message: dict):
        ...
        if not is_need or is_timeout:  # correct in one pass or refine success or timeout
            ...
            new_sql, reply = self._refine(query, evidence, schema_info, fk_info, error_info)
            message['try_times'] = message.get('try_times', 0) + 1
            message['pred'] = new_sql
            message['ori_refiner_reply'] = reply
            message['fixed'] = True
            message['send_to'] = REFINER_NAME
        return
