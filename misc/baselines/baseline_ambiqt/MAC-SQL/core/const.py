MAX_ROUND = 1  # max try times of one agent talk
...
Given a 【Database schema】 description, and the 【Question】, you need to use valid SQLite and understand the database, the ambiguous of table/column name, and then generate all corresponding probable SQL queries.
...
[
  ...
  (title, title. Value examples: ["Stark's Park", 'Somerset Park', 'Recreation Park', 'Hampden Park', 'Glebe Park', 'Gayfield Park'].),
  (appellation, appellation. Value examples: ["Stark's Park", 'Somerset Park', 'Recreation Park', 'Hampden Park', 'Glebe Park', 'Gayfield Park'].),
...
All probable SQL queries
```sql
[
SELECT T1.`Name`, COUNT(*) FROM stadium AS T1 JOIN concert AS T2 ON T1.`Stadium_ID` = T2.`Stadium_ID` GROUP BY T1.`Stadium_ID`;
SELECT T1.`title`, COUNT(*) FROM stadium AS T1 JOIN concert AS T2 ON T1.`Stadium_ID` = T2.`Stadium_ID` GROUP BY T1.`Stadium_ID`;
SELECT T1.`appellation`, COUNT(*) FROM stadium AS T1 JOIN concert AS T2 ON T1.`Stadium_ID` = T2.`Stadium_ID` GROUP BY T1.`Stadium_ID`
]
```
...
[
  ...
  (track_release_year, track release year. Value examples: ['2016', '2014'].),
  (composition_release_year, composition release year. Value examples: ['2016', '2014'].),
...
```sql
[
SELECT `Song_Name`, `track_release_year` FROM singer WHERE Age = (SELECT MIN(Age) FROM singer);
SELECT `Song_Name`, `composition_release_year` FROM singer WHERE Age = (SELECT MIN(Age) FROM singer)
]
```
