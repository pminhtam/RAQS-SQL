...
def init_spider_message(idx: int, item: dict) -> dict:
    ...
    """
    db_id, query, evidence, gt = item['db_id'], item['question'], str(""), item['orig_query']
    difficulty = eval_hardness(item['sql'])
    extra_map = item.get('extra_map', {})
    extra_table_map = item.get('extra_table_map', {})
    schema_without_content = item.get('schema_without_content', "")
    schema_with_content = item.get('schema_with_content', "")
    orig_example_idx = item.get('orig_example_idx', -1)
    orig_query = item.get('orig_query', "")
    user_message = {
        "idx": idx,
        "db_id": db_id,
        "query": query,
