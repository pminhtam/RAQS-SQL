...
Using embedding to get ambiguous values.
Using question embedding to query.
...
def embedding_search(question ,db_schema, client_chromadb):
    ...
    for table in db_schema:
        for column in db_schema[table]:
            ...
            column_lower = column_lower.replace(" ", "z").replace("/", "z").replace("(", "z").replace(")", "z").replace(
                "%", "z").replace("'", "z").replace("=", "z").replace(",",
                                                                      "z") + "z"  # Tên đổi từ script embedding_ambval.py
            collection_name = (db_id + table_que)[:(63 - len(column_lower))] + column_lower
            try:
                collection = client_chromadb.get_collection(collection_name)
            except:
                # print(f"Collection {collection_name} not found")
                continue
            relevant_docs = collection.query(query_texts=question,
                                             n_results=min(10,collection.count()))
            metadata = relevant_docs['metadatas'][0]
            match_content_dict[table + '.' + column] = []
            for idx, doc in enumerate(metadata):
                match_content_dict[table + '.' + column].append(doc["value"])
                # print(f"{idx} : {doc['value']}")
    match_content_str = ""
    for key in match_content_dict:
        match_content_str += f"{key} : {match_content_dict[key]}\n"
    return match_content_str
###################################################################################################
###################################################################################################
@func_set_timeout(10)
...
if __name__ == "__main__":
    ...
    # output_path = f"output_baseline_type2_{model_llm_api_name}.json"      # Cho local api
    output_path = f"output_baseline_type2_{model_llm_api_name}_evidence.json"      # Cho local api
    if os.path.exists(output_path):
        os.remove(output_path)

    embedding_path = "../EmbeddingTaskAmbiText2SQL/output_embedding_type2/"
    client_chromadb = chromadb.PersistentClient(path=f"{embedding_path}/")
    ...
    prompt_template = open("models/api_llm/templates/template_baseline_type2.txt", "r").read()
    idx = 0
    for item in items:
        db_id = item.db_id
        idx += 1
        ...
        matched_content = embedding_search(item.question, db_context_dict[db_id], client_chromadb)
        print(matched_content)
        request_kwargs = {
            "SCHEMA_STR": SCHEMA_STR,
            "QUESTION_TEXT": item.question,
            "MATCHED_CONTENTS": matched_content
        }
        ...
        print(idx ,result_eval, "pred_query:", pred_query)
        result = {
            "result": result_eval,
            "db_id": item.db_id,
...
"""
python -m baselines.baseline_ambival.baseline_type2_emb_worelval
"""