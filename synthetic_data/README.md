...
This directory contains the pipeline for generating **ambiguous value data** for the AmbiVal project. The pipeline extracts value entities from Text-to-SQL datasets (Spider, BIRD, WikiSQL), generates synonym and sub-class alternatives using an LLM, and produces modified databases and SQL queries for evaluation.
Data is publicly available at [https://huggingface.co/datasets/griffith-bigdata/ambival](https://huggingface.co/datasets/griffith-bigdata/ambival).
